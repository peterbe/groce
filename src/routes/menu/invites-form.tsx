...
import { FirestoreInvitation, Invitation, Menu } from "../../types";
...
export const InvitationsForm: FunctionalComponent<Props> = ({
  ...
  useEffect(() => {
    const ref = db
      .collection(`menus/${menu.id}/invitations`)
      .onSnapshot(
        (snapshot) => {
  ...
  function deleteInvite(invitationID: string) {
    db.collection(`menus/${menu.id}/invitations`)
      .doc(invitationID)
      .delete()
      .then(() => {
    ...
    <form>
      <div class="mb-3">
        ...
        {invitations && invitations.length ? (
          <div>
            {invitations.map((invitation) => {
              const inviteURL = `/invited/${menu.id}/${invitation.id}`;
                        ...
                        onClick={() => {
                          const shareData = {
                            ...
                            text: user?.displayName
                              ? `Menu (${menu.name}) shared by ${user.displayName}`
                              : `Menu (${menu.name})`,
        ...
        {invitations && !invitations.length && !user.isAnonymous && (
          <p>
            <i>You have no current invites to this menu</i>
          </p>
        )}
        {invitations && !invitations.length && user.isAnonymous && (
